...
static void packetDrop(FILE* stream, double current_time, uint myId) {
	// *stream->GetStream() << Simulator::Now ().GetSeconds () - current_time << "\t" << std::endl;
...
//
    ...
    Data1 = fopen ("q1_Yeah_TCP_H1_H4_file_q1.txt","w");
    CN_YeahTCP = fopen ("q1_Yeah_TCP_H1_H4_Congestion_data_q1.txt","w");
    ThroughPut_YeahTCP = fopen ("q1_Yeah_TCP_H1_H4_Throughput_data_q1.txt","w");
    GoodPut_YeahTCP = fopen ("q1_Yeah_TCP_H1_H4_Goodput_data_q1.txt","w"); 
	...
	//cout << "Ending the TCP_Yeah Simulation at time " << current_time << endl;
	cout << endl;

    FILE * Data2,*CN_Hybla,*ThroughPut_Hybla,*GoodPut_Hybla;
    Data2 = fopen ("q1_Hybla_H2_H5_file_q1.txt","w");
    CN_Hybla = fopen ("q1_Hybla_H2_H5_Congestion_data_q1.txt","w");
    ThroughPut_Hybla = fopen ("q1_Hybla_H2_H5_Throughput_data_q1.txt","w");
    GoodPut_Hybla = fopen ("q1_Hybla_H2_H5_Goodput_data_q1.txt","w"); 
    
	//tcpHybla Simulation
	std::cout << "Establishing TCP Hybla Simulation" << endl;
	Ptr<Socket> TcpHyblaSocket = HyblaFlow(InetSocketAddress(i_h5r2.GetAddress(0), port), port, nodes.Get(1), nodes.Get(6), current_time, current_time+elapsed_time, packetSize, totalPackets, "50Mbps", current_time, current_time+elapsed_time);
    ...
    //increment start TIme
	current_time += elapsed_time;
	//cout << "Ending the TCP_Hybla Simulation at time " << current_time << endl;
	cout << endl;

    //westWood+ Stimulation
    FILE * Data3,*CN_WestWood,*ThroughPut_Westwood,*GoodPut_Westwood;
    Data3 = fopen ("q1_Westwood_H3_H6_file_q1.txt","w");
    CN_WestWood = fopen ("q1_Westwood_H3_H6_Congestion_data_q1.txt","w");
    ThroughPut_Westwood = fopen ("q1_Westwood_H3_H6_Throughput_data_q1.txt","w");
    GoodPut_Westwood = fopen ("q1_Westwood_H3_H6_Goodput_data_q1.txt","w"); 

	std::cout << "Establishing TCP Westwood+ Simuation" << endl;
	Ptr<Socket> westwoodPSocket = WestwoodFlow(InetSocketAddress(i_h6r2.GetAddress(0), port), port, nodes.Get(2), nodes.Get(7), current_time, current_time+elapsed_time, packetSize, totalPackets, "50Mbps", current_time, current_time+elapsed_time);
    ...
    //increment start TIme
	current_time += elapsed_time;
	//cout << "Ending the TCP_Westwood+ Simulation at time " << current_time << endl;
	cout << endl;
	...
	Simulator::Stop(Seconds(current_time));
	Simulator::Run();
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i) {
        ...
        if (tempClassifier.sourceAddress=="102.115.1.1"){
			//std::cout << "Yeah_TCP Packet received " << endl;
			...
			//std::cout << "TCP_Hybla Packet received" << endl;
			...
			//std::cout << "TCP_Westwood+ Packet received" << endl;
